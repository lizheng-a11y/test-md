...
async function runWorkflow(fileId, orderId) {
  const res = await fetch(`${CONFIG.difyBaseUrl}/v1/workflows/run`, {
    ...
    body: JSON.stringify({
      inputs: {
        pdf: { transfer_method: 'local_file', upload_file_id: fileId, type: 'document' },
        OrderId: orderId,
      },
...
async function main() {
  ...
  const orders = fs.readdirSync(inputDir, { withFileTypes: true })
    .filter(d => d.isDirectory())
    .map(d => d.name);

  if (orders.length === 0) {
    console.log('No order folders found in inputFiles/');
    process.exit(0);
  }
  ...
  for (const orderId of orders) {
    const orderDir = path.join(inputDir, orderId);
    const pdfs = fs.readdirSync(orderDir).filter(f => f.toLowerCase().endsWith('.pdf'));
    if (pdfs.length === 0) {
      console.log(`\n[${orderId}] No PDF found, skipping`);
      continue;
    }
    const pdfPath = path.join(orderDir, pdfs[0]);

    console.log(`\n===== ${orderId} =====`);

    console.log(`[1/4] Uploading PDF...`);
    const fileId = await uploadPdf(pdfPath);
    console.log(`      done (file_id: ${fileId})`);

    console.log(`[2/4] Running workflow...`);
    const start = Date.now();
    const outputs = await runWorkflow(fileId, orderId);
