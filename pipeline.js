...
async function uploadToKnowledge(mdFiles) {
  ...
  for (const { filename, content } of mdFiles) {
    ...
    try {
      ...
      await setDocumentMetadata(datasetId, documentId, fieldIdCache[datasetId], orderId);
    ...
    // 同じファイルを（メータデータなし）版にも追加アップロード
    const noMetaDatasetId = CONFIG.knowledgeMap[`${prefix}（メータデータなし）`];
    if (noMetaDatasetId && !noMetaDatasetId.startsWith('DATASET_ID_')) {
      try {
        const text = content.toString('utf-8');
        await uploadByText(noMetaDatasetId, filename, text);
        console.log(`  ✓ ${filename} → ${noMetaDatasetId} (メータデータなし)`);
        success++;
      } catch (err) {
        console.error(`  ✗ ${filename} (メータデータなし) → ${err.message}`);
        failed++;
      }
    }
  }
...
async function main() {
  ...
  let targetOrderIds = null;

  if (args.includes('--retry')) {
    const logPath = fileURLToPath(new URL('./failed-orders.json', import.meta.url));
    if (!fs.existsSync(logPath)) {
      console.log('No failed-orders.json found, nothing to retry.');
      process.exit(0);
    }
    const log = JSON.parse(fs.readFileSync(logPath, 'utf-8'));
    targetOrderIds = new Set(log.failures.map(f => f.orderId));
    console.log(`Retrying ${targetOrderIds.size} failed order(s): ${[...targetOrderIds].join(', ')}`);
  } else if (args.length > 0) {
    targetOrderIds = new Set(args);
    console.log(`Processing specified order(s): ${[...targetOrderIds].join(', ')}`);
  }
  ...
  const ordersToProcess = targetOrderIds
    ? Object.entries(orderMap).filter(([id]) => targetOrderIds.has(id))
    : Object.entries(orderMap);

  if (ordersToProcess.length === 0) {
    console.log('No matching orders found in inputFiles/');
    process.exit(0);
  }
  ...
  for (const [orderId, filenames] of ordersToProcess) {
    ...
    try {
      ...
      const saveDir = fileURLToPath(new URL('./output/対応システム・対応内容', import.meta.url));
      if (!fs.existsSync(saveDir)) fs.mkdirSync(saveDir, { recursive: true });
      for (const { filename, content } of mdFiles) {
        if (filename.startsWith('対応システム・対応内容_')) {
          fs.writeFileSync(path.join(saveDir, filename), content);
          console.log(`      saved: ${filename}`);
        }
      }

      currentStage = 'knowledge-upload';
      ...
      if (failed > 0) {
        failedOrders.push({ orderId, stage: 'knowledge-upload', error: `${failed} file(s) failed`, failedAt: new Date().toISOString() });
        totalFailed += failed;
      }
      totalSuccess += success;
      totalSkipped += skipped;
    } catch (err) {
      console.error(`  ✗ FAILED [${orderId}] at stage [${currentStage}]: ${err.message}`);
      failedOrders.push({ orderId, stage: currentStage, error: err.message, failedAt: new Date().toISOString() });
      totalFailed++;
    }
  }
  ...
  if (failedOrders.length > 0) {
    fs.writeFileSync(logPath, JSON.stringify({ failures: failedOrders }, null, 2), 'utf-8');
    console.log(`\nFailed orders written to failed-orders.json`);
